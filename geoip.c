...
int main(int argc, char *argv[])
{
	...
	if((fp=fopen(QQWRY,"rb"))==NULL)
	{
		...
		if((fp=fopen(path,"rb"))==NULL)
		{
			...
			fprintf(stderr, "[-] Error : Please put %s in current or HOME directory.\n",QQWRY);
			return 2;
		}
	}
	...
	for (i=optind ; i<argc ; i++ ) {
		...
		printf("%lu.%lu.%lu.%lu\t",(ip&0xFF000000)>>0x18,(ip&0x00FF0000)>>0x10,(ip&0x0000FF00)>>0x8,ip&0x000000FF);
